#include "solosnake/blue/blue_examplegame.hpp"
#include <cassert>
#include <algorithm>
#include <vector>
#include <random>
...
#define SS_BOARD_BMP "example.bmp"
#define SS_HUESHIFT 127
#define SS_BOTSCRIPT "random-rotate-and-fire.txt"
#define SS_BOARDALPHA 0.6f
#define SS_BOARDLIGHT 0.25f
...
namespace blue
{
    ...
    bool examplegame::try_load_game(const shared_ptr<gamestartupargs>& args)
    {
        ...
        if (fullyLoaded_)
        {
            ss_err("load_game called more than once on same 'examplegame' object.");
            ss_throw("load_game called more than once on same 'examplegame' object.");
        }
        ...
        try
        {
                ...
                = [&](const char* scriptname, Hex_coord xy, HexFacingName f, const unsigned int botId)
            {
                auto bot = machinemaker->make_machine(
                    scriptname,
                    "thruster1", "thruster1",
                    "generic03", "generic03", "generic03", "generic03",
                    "generic03", "generic03", "generic03", "generic03",
                    "solarpanels", "solarpanels", "cpu", "capacitor", "capacitor",
                    nullptr);
            ...
            {
                std::srand(112233);
                unsigned int botnumber = 0u;

                for( uint8_t y = 0u; y < boardstate->grid_height(); ++y )
                {
                    for( uint8_t x = 0u; x < boardstate->grid_height(); ++x )
                    {
                        Hex_coord xy = Hex_coord::make_coord(x,y);
                        if( boardstate->machine_at_coord(xy) == nullptr )
                        {
                            if(  is_empty_tile(boardstate->grid().contents(xy)) )
                            {
                                if( (x * y) % 2 && (0 == (std::rand() % 3)) )
                                {
                                    machinemaker->set_hue_shift( (x * y) % 256 );

                                    switch( (x+y) % 5 )
                                    {
                                    case 0:
                                        addkillbot1( SS_BOTSCRIPT, xy, FacingTile0, botnumber++ );
                                        break;
                                    case 1:
                                        addkillbot2( SS_BOTSCRIPT, xy, FacingTile1, botnumber++ );
                                        break;
                                    case 2:
                                        addkillbot3( SS_BOTSCRIPT, xy, FacingTile2, botnumber++ );
                                        break;
                                    case 3:
                                        addkillbot4( SS_BOTSCRIPT, xy, FacingTile3, botnumber++ );
                                        break;
                                    case 4:
                                        addcargobot( SS_BOTSCRIPT, xy, FacingTile4, botnumber++ );
                                        break;
                                    }
                                }
                            }
            ...
            // Now we have all the examplegame state ready. Expose it to Lua
            ...
            // Now create the views containing the GUIs. The examplegame state is exposed
            ...
            solosnake::bgra boardTileColours[3] = {
                solosnake::bgra(221, 153, 187, static_cast<std::uint8_t>(255.0f * SS_BOARDALPHA)),
                solosnake::bgra(187, 153, 221, static_cast<std::uint8_t>(255.0f * SS_BOARDALPHA)),
                solosnake::bgra(153, 187, 221, static_cast<std::uint8_t>(255.0f * SS_BOARDALPHA))
            };

            solosnake::bgr ambientbgr( static_cast<std::uint8_t>(255.0f * SS_BOARDLIGHT) );
        ...
        {
            ss_err(e.what());
            assert(!"Exception while trying to load examplegame.");
        }
