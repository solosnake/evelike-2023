...
#include "solosnake/isun_cache.hpp"
...
namespace solosnake
{
    renderer_cache::renderer_cache( shared_ptr<ifilefinder> modelspaths,
                                    ...
                                    shared_ptr<isun_cache> suncache,
    ...
    modelnode renderer_cache::recursively_construct_model( const modelnodedescription& desc )
    {
        ...
        {
            node.add_child_node( move( recursively_construct_model( desc.child_node( i ) ) ) );
        }
    ...
    void renderer_cache::load_suntype_file( const string& file )
    {
        suntypes_->load_sun_file( file );
    }
    ...
    solosnake::sunhandle_t renderer_cache::get_sun_type( const std::string& name )
    {
        auto i = knownsuntypes_.find( name );

        if( i != knownsuntypes_.end() )
        {
            return i->second;
        }

        auto st = suntypes_->get_sun_type( name );
        auto handle = renderer_->load_sun( *st );

        knownsuntypes_.insert( pair<string, sunhandle_t>( name, handle ) );
