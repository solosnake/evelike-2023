...
int tiny_xml_test()
{
...

    {
        ...
        outputStream << doc;
        XmlTest(
            "Output stream correct.", string( demoEnd ).c_str(), outputStream.str().c_str(), true );
#endif
...
#ifdef TIXML_USE_STL
        {
            ...
            XmlTest(
                "Reads attribute 'attribute0=\"foo0\"'.", "foo0", element0.Attribute( "attribute0" ) );
    ...
    TiXmlBase::SetCondenseWhiteSpace( false );
    {
        ...
        XmlTest(
            "Condense white space OFF.", "This  is    \ntext", text1.FirstChild()->Value(), true );
    }
...

    {
        ...
        const char* expected = "Line 5 has \"quotation marks\" and 'apostrophe "
                               "marks'. It also has <, >, and &, as well as a "
                               "fake copyright \xC2\xA9.";
...

    {
        ...
        if( textfile )
        {
            ...
            XmlTest(
                "Entity with one digit.", text->Value(), "1.1 Start easy ignore fin thickness\n" );
        }
    }
...

    {
        ...
        XmlTest( "Parsing repeated attributes.", true, doc.Error() ); // is an  error to tinyxml
        // (didn't use to be, but
        // caused issues)
...

    {
        ...
        XmlTest(
            "Document only at top level.", xml.ErrorId(), TiXmlBase::TIXML_ERROR_DOCUMENT_TOP_ONLY );
    }
...
#ifdef TIXML_USE_STL
    {
        ...
        XmlTest(
            "Comments ignore entities.", " declarations for <head> & <body> ", c0->Value(), true );
