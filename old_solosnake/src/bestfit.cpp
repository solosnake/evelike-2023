...
namespace solosnake
{
    namespace bestfit
    {
        namespace
        {
            ...
            unique_ptr<solutionset> try_pack_rects_into_rect(
                    ...
                    const vector<indexedrect>& rects )
            {
                ...
                for( size_t i = 0; i < rects.size(); ++i )
                {
                    if( false == try_place_rect_leftmost( rects[i], sln ) )
                    {
                        ...
                        return unique_ptr<solutionset>();
                    }
                }

                return unique_ptr<solutionset>( new solutionset( sln ) );
            }
            ...
            solutionset find_best_fit( vector<indexedrect> rects )
            {
                ...
                while( currentwidth > widestRect )
                {
                    if( area( currentwidth, currentheight ) < bestsofar.area() )
                    {
                        ...
                        // area....
                        auto fittedrects = try_pack_rects_into_rect( currentwidth, currentheight, rects );
