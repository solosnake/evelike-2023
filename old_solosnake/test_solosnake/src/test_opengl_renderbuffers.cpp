...
#define BUFFER_OFFSET(i) ((char*)NULL + (i))
    ...
    void CreateLightingFBOs()
    {
        assert( GL_NO_ERROR == glGetError() );

        glGenRenderbuffers( 1, &rboSpec );
        glBindRenderbuffer( GL_RENDERBUFFER, rboSpec );
        glRenderbufferStorage( GL_RENDERBUFFER, GL_RGBA8, 640, 480 );

        assert( GL_NO_ERROR == glGetError() );

        glGenRenderbuffers( 1, &rboDiff );
        glBindRenderbuffer( GL_RENDERBUFFER, rboDiff );
        glRenderbufferStorage( GL_RENDERBUFFER, GL_DEPTH_COMPONENT32, 640, 480 );

        assert( GL_NO_ERROR == glGetError() );

        glGenFramebuffers( 1, &fboLights );
        glBindFramebuffer( GL_DRAW_FRAMEBUFFER, fboLights );
        glFramebufferRenderbuffer( GL_DRAW_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, rboSpec );
        glFramebufferRenderbuffer( GL_DRAW_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_RENDERBUFFER, rboDiff );
    ...
    void CreateGeometryFBOs()
    {
        ...
        // glTexParameteri(GL_TEXTURE_2D, GL_GENERATE_MIPMAP, GL_TRUE); // automatic
        // mipmap
        glTexImage2D( GL_TEXTURE_RECTANGLE, 0, GL_RGBA8, 640, 480, 0, GL_RGBA, GL_UNSIGNED_BYTE, 0 );
        ...
        glFramebufferTexture2D(
            GL_DRAW_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_RECTANGLE, textureId, 0 );
