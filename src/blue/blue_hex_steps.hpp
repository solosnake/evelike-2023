...
namespace blue
{
    ...
    /// minimum steps needed to move from start to end. If the callback
    /// function returns false then the algorithm is stopped and no more tiles
    /// are processed.
    /// See http://zvold.blogspot.se/2010/02/line-of-sight-on-hexagonal-grid.html
    void hex_steps_under_line( const int startx,
                               const int starty,
                               const int endx,
                               const int endy,
